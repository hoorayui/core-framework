...
type DataSource func() []interface{}

func init() {
	builtins.LoadAllBuiltins()
}

func (md *MemDriver) setupDB() (*sql.DB, error) {
	fNumber := md.dt.NumField()
	cols := make([]string, 0, fNumber)
	for i := 0; i < fNumber; i++ {
		f := md.dt.Field(i)
		cols = append(cols, f.Name)
	}
	// 反射为memdb
	mdb, err := memdb.NewMemDb(md.dbTableName, cols)
	if err != nil {
		return nil, err
	}
	err = schema.RegisterSourceAsSchema(md.dbTableName, mdb)
	if err != nil {
		panic(err)
	}
	db, err := sql.Open("qlbridge", md.dbTableName)
...
// NewMemDriver create mem driver
func NewMemDriver(dt reflect.Type, ds DataSource) *MemDriver {
	tableName := dt.Name()
	tableName += "_" + randString(5)
	tableName = strings.ToLower(tableName)
	return &MemDriver{dt: dt, ds: ds, dbTableName: tableName}
}

// FindRows ...
func (md *MemDriver) FindRows(ctx context.Context, _ *mysql.Session, tmd registry.TableMetaData, conditions []*driver.Condition,
	...
	query := "SELECT %s FROM " + md.dbTableName
	countQuery := "SELECT COUNT(*) FROM " + md.dbTableName
	aggQuery := "SELECT %s FROM " + md.dbTableName
	...
	if len(aggregateColumns) > 0 {
		aggRow := db.QueryRow(aggQuery, aggQueryArgs...)
		if aggRow != nil {
	...
	row := db.QueryRow(countQuery, queryArgs...)
	if row != nil {
	...
	// parse data
	for rows.Next() {
		var selectRow = reflect.New(tmd.RowDataType()).Interface()
		err = rows.Scan(selectRow)
		if err != nil {
			return nil, errors.Wrap(err).Log()
		}
